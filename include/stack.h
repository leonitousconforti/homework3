...
class Stack {
 ...
 public:
  ...
  // the first element listed will be at the top of the stack (first to get
  // popped off) the second element will be the second thing to get popped off
  // and so on
  ...
  // == operator overload
  // returns true if both stacks contain all of the same elements in the same
  // order capacity does not matter
  ...
  // exits with exit code EXIT_FAILURE and prints an error message to cout if
  // stack is empty
  int pop();

  // pushes x onto the top of the stack
  // exits with exit code EXIT_FAILURE and prints an error message to cout if
  // stack is full returns a reference to the current stack
  Stack& push(int x);
  ...
  // maps the function f over all the elements
