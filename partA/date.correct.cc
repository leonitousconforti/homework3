...
/**
 * Constructor
 * param [in] year, month, day: integer values for candidate date's year, month,
 * day respectively
 *
 * The easiest way to determine if the inputs are valid, is to convert to days
 * and back.
 **/
Date::Date(int year, int month, int day) {
  int candidate_date = ConvertToDays(year, month, day);
  // std::cout << "Debugging ConvertToDays: " << old_date << std::endl;
  int* new_date = ConvertFromDays(candidate_date);
  if (year == new_date[0] && month == new_date[1] && day == new_date[2]) {
    yyyy_ = new_date[0];
    mm_ = new_date[1];
    dd_ = new_date[2];
  } else {
...
/**
 * Constructor
 * param [in] epoch: an approximation of the number of seconds that have elapsed
 * since 00:00:00 Coordinated Universal Time (UTC), Thursday, 1 January 1970
 *
 * Converts epoch time given to this Date format. Uses C std gmtime to convert
 * epoch time to tm structure
 **/
Date::Date(int epoch) {
 ...
 * param [out] New Date object representing the date which is "days" number of
 * days after the date currently stored MUTATION : THIS VERSION DOES NOT TAKE
 ...
 * param [in] rhs: right hand side of this+rhs
 * param [out] New Date object representing the date which is "days" number of
 * days before the date currently stored
 **/
Date Date::operator-(int days) const {
  int old_date = ConvertToDays(yyyy_, mm_, dd_);
  int* new_date = ConvertFromDays(old_date - days);
...
std::string Date::GetDate() const {
  ...
  date = date + numstr + "-";
  if (mm_ < 10) date = date + "0";
  sprintf(numstr, "%d", mm_);
  date = date + numstr + "-";
  if (dd_ < 10) date = date + "0";
