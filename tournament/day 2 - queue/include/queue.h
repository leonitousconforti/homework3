...
class Queue {
 ...
 public:
  ...
  int dequeue();  // pop 0th element from the queue and return it, exits if
                  // queue is empty
  void enqueue(int x);  // add a new element 'int x' to the end of the queue,
                        // exits if queue is full
  ...
  void clear();  // clears the contents of the queue, capacity stays the same,
                 // size changes
